...
// Whenever data is expected in the body
// the body should be defined and should not be an expected object
function validateBody(req, res, next) {
  // if (req.body && Object.keys(req.body).length > 0) {
  //   next();
  // } else {
  //   res.status(400).json({ message: 'body pls' });
  // }
  req.body && Object.keys(req.body).length > 0
    ? next()
    : // : res.status(400).json({ message: 'please include a body' });
      next({ message: 'please include the request body' });
}
